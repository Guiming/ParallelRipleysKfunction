...
public: 
	...
	void computeKr(int nr, double* rs, int num_threads, bool hash, int idx_range[], double* counts); // the value of Ripley's K function given distances rs without edge correction
	double computeKr_vec(double r);
	void computeKr_vec(int nr, double* rs, int num_threads, bool hash, int idx_range[], double* counts_total);
	double getWidth();
	double getHeight();
	double getRmax();
	Window* getWindow();
private:
	void setNumberOfPoints(int n); // set number of points
...
//////////////////// for MPI ///////////////////////
public:
	void packPoints(double* xcoords, double* ycoords);
	void packWindow(double* wxcoords, double* wycoords);
	//void packLookupTable(double* distances, double* weights); //should be in Point.h
public:
	SpatialPointPattern(int np, double* xcoords, double* ycoords, double maxDist,
		int type, int nw, double* wxcoords, double* wycoords);
public:
	void unpackPoints(int n, double* xcoords, double* ycoords, double maxDist);
	void unpackWindow(int type, int n, double* wxcoords, double* wycoords);
	//void unpackLookupTable(double* distances, double* weights); //should be in Point.h
};
